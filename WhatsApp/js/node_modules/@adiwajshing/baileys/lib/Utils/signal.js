require((exports, module) => {
    ...
    const signalStorage = (auth) => ({
        loadSession: async (id) => {
            const { [id]: sess } = await auth.keys.get('session', [id]);
            if (sess) {
        ...
        removePreKey: (id) => auth.keys.set({ 'pre-key': { [id]: null } }),
        loadSignedPreKey: () => {
        ...
        getOurRegistrationId: () => (auth.creds.registrationId),
        getOurIdentity: () => {
            const { signedIdentityKey } = auth.creds;
            return {
