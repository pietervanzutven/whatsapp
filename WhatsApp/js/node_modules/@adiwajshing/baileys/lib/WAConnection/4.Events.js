...
const QR = __importStar(require("qrcode-terminal/lib/main"));
const _3_Connect_1 = require("./3.Connect");
const Constants_1 = require("./Constants");
const Utils_1 = require("./Utils");
const keyed_db_1 = __importDefault(require("@adiwajshing/keyed-db/lib/KeyedDB"));
...
class WAConnection extends _3_Connect_1.WAConnection {
    constructor() {
        ...
        this.emitParticipantsUpdate = (jid, participants, action) => {
            ...
            if (meta) {
                switch (action) {
                    case 'add':
                        participants.forEach(jid => (meta.participants.push(Object.assign({}, this.contactAddOrGet(jid), { isAdmin: false, isSuperAdmin: false }))));
                        break;
                    case 'remove':
        ...
        this.emitGroupUpdate = (jid, update) => {
            ...
            this.emit('group-update', Object.assing({}, { jid }, update ));
        };
            ...
            chats.all().forEach(chat => {
                ...
                if (!oldChat) {
                    ...
                    if (oldChat.t !== chat.t || oldChat.modify_tag !== chat.modify_tag) {
                        ...
                        updatedChats.push(Object.assign({}, changes, { jid: chat.jid }));
                    }
                }
            });
            this.chats = chats;
                ...
                messages.reverse().forEach(([, , message]) => {
                    ...
                    if (chat) {
                        ...
                        if (chat.messages.upsert(message).length > 0) {
                            overlaps[jid] = Object.assign({}, (overlaps[jid] || { requiresOverlap: true }), { didOverlap: true });
                        }
            ...
            json[2].forEach(([type, contact]) => {
                ...
                if (presentContact) {
                    ...
                    if (changes && Object.keys(changes).length > 0) {
                        updatedContacts.push(Object.assign({}, changes, { jid: contact.jid }));
                    }
                }
                else
                    updatedContacts.push(contact);
                contacts[contact.jid] = Object.assign({}, (presentContact || {}), contact );
            });
            // update chat names
            const updatedChats = [];
            ...
            if (data) {
                ...
                switch (data[0]) {
                    ...
                    case "demote":
                        ...
                        break;
                    case "desc_add":
                        emitGroupUpdate(Object.assign({}, data[2], { descOwner: data[1] }));
                        break;
                    default:
    ...
    applyingPresenceUpdate(update) {
        ...
        if (chat && jid.endsWith('@s.whatsapp.net')) { // if its a single chat
            ...
            const presence = Object.assign({}, (chat.presences[jid] || {}) );
            if (update.t)
    ...
    async chatAddMessageAppropriate(message) {
        const jid = Utils_1.whatsappID(message.key.remoteJid);
        if (Utils_1.isGroupID(jid) && !jid.includes('-')) {
